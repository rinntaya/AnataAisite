	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				// profiling output.
				if (Aisite::Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					AT_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Aisite::Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					AT_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define HZ_PROFILE 0
	...
	#define AT_PROFILE_BEGIN_SESSION(name, filepath) ::Hazel::Instrumentor::Get().BeginSession(name, filepath)
	#define AT_PROFILE_END_SESSION() ::Hazel::Instrumentor::Get().EndSession()
	...
	#define AT_PROFILE_FUNCTION() HZ_PROFILE_SCOPE(HZ_FUNC_SIG)
#else
