...
namespace Aisite {
	...
	struct SceneUtils {
		template<typename F>
		static void forEachComponent(F&& f) {
			(f.template operator()<Components>(), ...);  // C++17 折叠表达式
		}
	};
	using ComponentsUtils = SceneUtils<
		TransformComponent,
	...
	static void CopyComponent(entt::registry& dst, entt::registry& src, const std::unordered_map<UUID, entt::entity>& enttMap)
	{
		...
		for (auto e : view)
		{
			...
			auto& component = src.get<C>(e);
			dst.emplace_or_replace<C>(dstEnttID, component);
		}
	}
	Ref<Scene> Scene::Copy(const Ref<Scene> &other)
	{
		...
		ComponentsUtils::forEachComponent([&]<typename T> {
	...
	void Scene::OnUpdateCamera(const Timestep ts)
	{
		const Camera* mainCamera = nullptr;
		glm::mat4 cameraTransform;
		{
	...
	void Scene::DuplicateEntity(Entity entity)
	{
		...
		ComponentsUtils::forEachComponent([&]<typename T>() {
			CopyComponentIfExists<T>(newEntity, entity);
		});
	}
	...
	template<>
	AISITE_API void Scene::OnComponentAdded<CircleRendererComponent>(Entity entity, CircleRendererComponent& component)
	{
	}
	template<>
	...
	template<>
	AISITE_API void Scene::OnComponentAdded<Rigidbody2DComponent>(Entity entity, Rigidbody2DComponent& component)
	{
	}

	template<>
	AISITE_API void Scene::OnComponentAdded<BoxCollider2DComponent>(Entity entity, BoxCollider2DComponent& component)
	{
	}

	template<>
	AISITE_API void Scene::OnComponentAdded<CircleCollider2DComponent>(Entity entity, CircleCollider2DComponent& component)
	{
	}
