...
namespace Aisite
{
    ...
    void OpenGLFramebuffer::Invalidate()
    {
        ...
        if (m_ColorAttachments.size() > 1) {
            AT_CORE_ASSERT(m_ColorAttachments.size() <= 4);
            GLenum buffers[4] = {
        ...
        AT_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE,
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
    {
        if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize) {
            AT_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
            return;
        }
    ...
    {
        AT_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
